...
//     Response response = Dispatcher::dispatch(request, server, NULL);
...
int main()
{
    ...
    request.body = "Hello Salah! >> Zanana";

    Response response = Dispatcher::dispatch(request, server, NULL);


    std::cout << response.getStatusCode()<< " "<< response.getReasonPhrase()<< std::endl;
